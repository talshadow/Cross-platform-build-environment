...
#
# Використання:
#   ./scripts/get-sysroot-yocto.sh --method sdk     --installer poky-*.sh [--dest /opt/poky]
#   ./scripts/get-sysroot-yocto.sh --method extract --installer poky-*.sh --dest /srv/yocto-sysroot
...
usage() {
    cat <<'EOF'
Використання: get-sysroot-yocto.sh --method <метод> [ОПЦІЇ]
...
method_sdk() {
    ...
    if [[ ${#env_scripts[@]} -eq 0 ]]; then
        ...
        echo "  # або:"
        echo "  ./scripts/build.sh yocto-release"
    fi
}
...
method_check() {
    ...
    if $ok; then
        ...
        echo "  ./scripts/get-sysroot-yocto.sh --method sdk --installer poky-*.sh"
        exit 1
    fi
}
