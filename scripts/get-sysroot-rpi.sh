...
#   METHOD=live    — синхронізує sysroot з живого RPi по SSH (→ sync-sysroot.sh)
#
# Використання:
#   ./scripts/get-sysroot-rpi.sh --method image  --image rpi.img --dest /srv/rpi4-sysroot
...
usage() {
    cat <<'EOF'
Використання: get-sysroot-rpi.sh --method <метод> --dest <шлях> [ОПЦІЇ]
...
Методи:
  ...
  live    Синхронізувати sysroot з живого RPi по SSH (обгортка sync-sysroot.sh)
...
Приклади:
  ...
  ./scripts/get-sysroot-rpi.sh --method image --image xxx.img.xz --dest /srv/rpi4-sysroot
...
# ===========================================================================
# МЕТОД 3: Живий RPi (делегація до sync-sysroot.sh)
# ===========================================================================
method_live() {
    [[ -z "${RPI_HOST}" ]] && { log_error "--host обов'язковий для method=live"; exit 1; }

    local SYNC_SCRIPT="${SCRIPT_DIR}/sync-sysroot.sh"
    if [[ ! -f "${SYNC_SCRIPT}" ]]; then
        log_error "scripts/sync-sysroot.sh не знайдено"
        exit 1
    fi
...
fixup_symlinks() {
    ...
    echo "  # або:"
    echo "  ./scripts/build.sh rpi4-release -DRPI_SYSROOT=${DEST}"
}
