...
function(_ep_binarydeps_build_search_dirs)
    ...
    if(CMAKE_SYSROOT)
        foreach(_base "${CMAKE_SYSROOT}/lib" "${CMAKE_SYSROOT}/usr/lib")
            ...
            # Рекурсивно всі підкаталоги будь-якої глибини:
            # multiarch (aarch64-linux-gnu/), Debian non-standard sub-libs
            # (openblas-pthread/, blas/, lapack/, dri/ тощо).
            file(GLOB_RECURSE _sr_sub_items LIST_DIRECTORIES true "${_base}/*")
            foreach(_item IN LISTS _sr_sub_items)
                if(IS_DIRECTORY "${_item}")
                    list(APPEND _sr_dirs "${_item}")
                endif()
            endforeach()
            unset(_sr_sub_items)
        endforeach()
