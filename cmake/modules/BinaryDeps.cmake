...
function(_ep_binarydeps_build_search_dirs)
    ...
    if(CMAKE_SYSROOT)
        foreach(_base "${CMAKE_SYSROOT}/lib" "${CMAKE_SYSROOT}/usr/lib")
            ...
            # Debian non-standard sub-libs (openblas-pthread/, blas/, lapack/ тощо):
            # Ці директорії на два рівні вглиб і не знаходяться першим glob.
            foreach(_m IN LISTS _multiarch_dirs)
                file(GLOB _subdirs LIST_DIRECTORIES true "${_m}/*")
                foreach(_sub IN LISTS _subdirs)
                    if(IS_DIRECTORY "${_sub}")
                        list(APPEND _sr_dirs "${_sub}")
                    endif()
                endforeach()
            endforeach()
        endforeach()
