...
macro(cross_toolchain_find_versioned_native_gcc VERSION VERSION_VAR)
    ...
    if(_vng_ar)
        set(CMAKE_AR     "${_vng_ar}"     CACHE FILEPATH "Archiver (LTO-aware)"  FORCE)
    endif()
    if(_vng_ranlib)
        set(CMAKE_RANLIB "${_vng_ranlib}" CACHE FILEPATH "Ranlib (LTO-aware)"    FORCE)
    endif()
    if(_vng_nm)
        set(CMAKE_NM     "${_vng_nm}"     CACHE FILEPATH "NM (LTO-aware)"        FORCE)
    endif()
    unset(_vng_ar)
    unset(_vng_ranlib)
    unset(_vng_nm)
...
macro(cross_toolchain_find_versioned_cross_compiler PREFIX VERSION)
    ...
    if(_vcc_cc AND _vcc_cxx)
        ...
        if(_vcc_ar)
            set(CMAKE_AR     "${_vcc_ar}"     CACHE FILEPATH "Archiver" FORCE)
        endif()
        if(_vcc_strip)
            set(CMAKE_STRIP  "${_vcc_strip}"  CACHE FILEPATH "Strip"    FORCE)
        endif()
        if(_vcc_ranlib)
            set(CMAKE_RANLIB "${_vcc_ranlib}" CACHE FILEPATH "Ranlib"   FORCE)
        endif()
        unset(_vcc_ar)
        unset(_vcc_strip)
        unset(_vcc_ranlib)
