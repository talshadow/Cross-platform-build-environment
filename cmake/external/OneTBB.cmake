...
if(USE_SYSTEM_ONETBB)
    ...
    elseif(EXISTS "${_tbb_lib}")
        ...
        if(ONETBB_USE_GIT)
            ...
            if(EXISTS "${EP_SOURCES_DIR}/onetbb/CMakeLists.txt")
                set(_s "${CMAKE_BINARY_DIR}/onetbb_ep-prefix/src/onetbb_ep-stamp")
                if(NOT EXISTS "${_s}/onetbb_ep-download")
                    file(MAKE_DIRECTORY "${_s}")
                    file(WRITE "${_s}/onetbb_ep-download" "")
                    message(STATUS "[oneTBB] Джерела вже є у ${EP_SOURCES_DIR}/onetbb — download stamp створено (пропускаємо завантаження)")
                endif()
                unset(_s)
            endif()
        endif()
